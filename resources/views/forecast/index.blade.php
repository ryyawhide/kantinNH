    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-header">
                ...
                <button id="btn-delete-all" class="btn btn-danger float-right">
                    ...
                    <table id="forecast_table" class="display w-100">
                        <thead>
                            <tr>
                                ...
                                <th style="min-width: 100px;">Periode Bulan</th>
...
$(document).ready(function() {
    ...
    $('#btn-generate').click(function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                // Set filter to the selected periode after generation
                $('#filter_periode').val(periode);
                loadForecasts();
                loadAnalytics();
                $('#barang_id').val('');
                $('#periode').val('');
    ...
    // Load Forecasts (with optional periode filter)
    function loadForecasts() {
        let periodeFilter = $('#filter_periode').val();

        $.ajax({
            url: '/forecast/get-data',
            type: 'GET',
            dataType: 'JSON',
            data: { periode: periodeFilter },
            success: function(response) {
                let counter = 1;
    ...
    $('#btn-delete-all').click(function() {
        Swal.fire({
            title: 'Hapus Semua Forecast?',
            text: 'Semua data forecast akan dihapus secara permanen! Tindakan ini tidak dapat dibatalkan.',
            icon: 'warning',
            showCancelButton: true,
            ...
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    error: function(error) {
                        Swal.fire('Error', 'Gagal hapus semua forecast!', 'error');
                    }
                });
            }
        });
    ...
    // Reload forecasts when periode filter changes
